...
def feedback_funcionario():
    ...
    data_inicial = request.args.get('data_inicial')
    data_final = request.args.get('data_final')
    ordenar = request.args.get('ordenar', 'desc')  # nova linha
    ...
    # nova lógica para ordenar por data
    if ordenar == 'asc':
        query = query.order_by(Importacao.data.asc())
    else:
        query = query.order_by(Importacao.data.desc())
    ...
    return render_template(
        ...
        media=media,
        ordenar=ordenar  # <-- passa isso pro template
    )
